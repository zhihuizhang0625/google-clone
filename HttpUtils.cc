...
bool is_path_safe(const string &root_dir, const string &test_file) {
  ...
  char root_dir_path[PATH_MAX];
  if (realpath(test_file.c_str(), test_file_path) == NULL || realpath(root_dir.c_str(), root_dir_path) == NULL){
      return false;
  }
  string root_string(root_dir_path);
  string test_string(test_file_path);
  root_string+='/';
  string substr = test_string.substr(0, root_string.length());
  return root_string.compare(substr) == 0;
}
